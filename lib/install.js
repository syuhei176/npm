...
function updateStatsFile (itWorked) {
  // If the stats reporter hasn't managed to PUT yet then kill it so that it doesn't
  // step on our updating the cli-stats json
  ...
  try {
    ...
    stats.metrics.to = new Date().toISOString()
    if (itWorked) {
      ++stats.metrics.successfulInstalls
    } else {
      ++stats.metrics.failedInstalls
    }
  } catch (ex) {
    stats = {
      metricId: uuid.v4(),
  ...
  try {
    ...
    // we couldn't write the error stats file, um, well, oh well.
  }
}
